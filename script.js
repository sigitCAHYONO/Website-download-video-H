document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... setelah const cancelMediaButton = ...
    const recordVnButton = document.getElementById('recordVnButton');
    const inputAreaRegular = document.querySelector('.input-area-regular');
    const inputAreaRecording = document.querySelector('.input-area-recording');
    const recordingTimer = document.getElementById('recordingTimer');

    // ... setelah let pendingMediaDataURL = null;
    let mediaRecorder;
    let audioChunks = [];
...
// --- LOGIKA UNTUK FITUR VOICE NOTE (VN) ---
    ...
    try {
        ...
        mediaRecorder.onstop = () => {
            ...
            reader.onloadend = () => {
                ...
                sendGroupMessage(null, vnPayload);
            };
            audioChunks = [];
            stream.getTracks().forEach(track => track.stop()); // Matikan stream mikrofon
        };

        // UI update
        inputAreaRegular.style.display = 'none';
        inputAreaRecording.style.display = 'flex';
        ...
        console.error("Error saat mengakses mikrofon:", err);
        addMessageToChatUI("Gagal mengakses mikrofon. Pastikan Anda memberikan izin.", 'system-error', true);
    }
}
...
function stopRecording() {
    ...
    inputAreaRegular.style.display = 'flex';
    inputAreaRecording.style.display = 'none';
    recordingTimer.textContent = "00:00";
}

if (recordVnButton) {
    recordVnButton.addEventListener('mousedown', startRecording);
    recordVnButton.addEventListener('mouseup', stopRecording);
    recordVnButton.addEventListener('touchstart', (e) => { e.preventDefault(); startRecording(); });
    recordVnButton.addEventListener('touchend', stopRecording);
    recordVnButton.addEventListener('mouseleave', () => { // Jika mouse keluar area tombol saat merekam
        if (mediaRecorder && mediaRecorder.state === 'recording') {
            stopRecording();
        }
    });
...
// --- AKHIR LOGIKA VN ---
    ...
    groupChatMessageInput.addEventListener('input', () => {
        // Logika untuk mengetik
        if (!websocket || websocket.readyState !== WebSocket.OPEN) return;
        clearTimeout(typingTimer);
        websocket.send(JSON.stringify({ type: 'typing', nickname: userNickname, isTyping: true }));
        typingTimer = setTimeout(() => {
            websocket.send(JSON.stringify({ type: 'typing', nickname: userNickname, isTyping: false }));
        }, TYPING_TIMER_LENGTH);

        // Logika untuk menampilkan tombol kirim vs rekam (BARU)
        if (groupChatMessageInput.value.trim() !== '') {
            recordVnButton.style.display = 'none';
            sendGroupChatMessageButton.style.display = 'flex';
        } else {
            recordVnButton.style.display = 'flex';
            sendGroupChatMessageButton.style.display = 'none';
        }
    });
}
