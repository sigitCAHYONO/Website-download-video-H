document.addEventListener('DOMContentLoaded', () => {
    ...
    const enterTiktokFeedButton = document.getElementById('enterTiktokFeedButton');
    const tiktokFeedSection = document.getElementById('tiktokFeedSection');
    const uploadContentButton = document.getElementById('uploadContentButton');
    const contentUploadInput = document.getElementById('contentUploadInput');
    ...
    function showHomeView() {
        ...
        tiktokFeedSection.style.display = 'none'; // <-- Ini baris yang penting
...
function showTiktokFeedView() {
    ...
    groupChatInputBar.style.display = 'none';
    replyContextBar.style.display = 'none';
    tiktokFeedSection.style.display = 'flex'; // Tampilkan feed
    currentView = 'tiktokfeed';

    appNameTitle.textContent = "Jelajahi Konten";
    appStatus.textContent = "Postingan terbaru dari pengguna";

    // Minta data feed dari server
    if (websocket && websocket.readyState === WebSocket.OPEN) {
        websocket.send(JSON.stringify({ type: 'request_feed' }));
    } else {
        tiktokFeedSection.innerHTML = '<p style="color: var(--text-secondary);">Gagal terhubung ke server untuk memuat konten.</p>';
    }
}

function renderFeed(posts) {
    tiktokFeedSection.innerHTML = ''; // Kosongkan feed sebelum render ulang
    if (posts && posts.length > 0) {
        posts.forEach(post => addPostToFeedUI(post, false));
    } else {
        tiktokFeedSection.innerHTML = '<p style="color: var(--text-secondary);">Belum ada postingan. Jadilah yang pertama!</p>';
    }
}

function addPostToFeedUI(post, prepend = false) {
    const postCard = document.createElement('div');
    postCard.className = 'post-card';
    postCard.id = post.id;

    // Mendapatkan URL gambar profil pengunggah
    const userPic = userProfilePictures[post.nickname] || DEFAULT_GROUP_AVATAR_URL;

    // Format waktu
    const postDate = new Date(post.timestamp);
    const timeString = postDate.toLocaleString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });

    let mediaElement;
    if (post.file_type === 'image') {
        mediaElement = `<img src="${post.file_path}" alt="Postingan gambar oleh ${post.nickname}">`;
    } else {
        mediaElement = `<video src="${post.file_path}" controls></video>`;
    }

    postCard.innerHTML = `
        <div class="post-header">
            <img src="${userPic}" class="profile-pic-bubble">
            <div class="post-user-info">
                <div class="post-nickname">${sanitizeHtml(post.nickname)}</div>
        ...
        ${post.caption ? `<div class="post-caption">${linkify(sanitizeHtml(post.caption))}</div>` : ''}
    `;

    if (prepend) {
        tiktokFeedSection.prepend(postCard);
    } else {
        tiktokFeedSection.appendChild(postCard);
    }
}
...
function showUploadPreview(file) {
    ...
    confirmUploadButton.textContent = 'Unggah';
...
function hideUploadPreview() {
    ...
    URL.revokeObjectURL(uploadPreviewImage.src);
    URL.revokeObjectURL(uploadPreviewVideo.src);
    uploadPreviewImage.src = '#';
    uploadPreviewVideo.src = '';
    contentUploadInput.value = null; // Reset input file
    pendingUploadFile = null;
}

async function handleConfirmUpload() {
    if (!pendingUploadFile || !userNickname) return;
    
    confirmUploadButton.disabled = true;
    uploadStatusMessage.textContent = 'Mengunggah...';

    const formData = new FormData();
    formData.append('content', pendingUploadFile);
    formData.append('nickname', userNickname);
    formData.append('caption', uploadCaptionInput.value.trim());
    ...
    try {
        ...
        if (result.success) {
            uploadStatusMessage.textContent = 'Unggah berhasil!';
            setTimeout(() => {
                hideUploadPreview();
            }, 1500);
        } else {
            uploadStatusMessage.textContent = `Gagal: ${result.message}`;
            confirmUploadButton.disabled = false;
        }
    } catch (error) {
        console.error('Error saat unggah:', error);
        uploadStatusMessage.textContent = 'Error jaringan. Coba lagi.';
        confirmUploadButton.disabled = false;
    }
}

// 5. Tambahkan event listener baru
if (enterTiktokFeedButton) {
    enterTiktokFeedButton.addEventListener('click', showTiktokFeedView);
}
...
if (uploadContentButton) {
    uploadContentButton.addEventListener('click', () => {
        contentUploadInput.click();
    });
}
if (contentUploadInput) {
    contentUploadInput.addEventListener('change', (event) => {
        const file = event.target.files[0];
        if (file) {
            const maxSize = file.type.startsWith('image/') ? 20 * 1024 * 1024 : 50 * 1024 * 1024; // 20MB for images, 50MB for videos
            if (file.size > maxSize) {
                alert(`Ukuran file terlalu besar! Maksimum ${maxSize / 1024 / 1024}MB.`);
                return;
            }
...
if(cancelUploadButton) cancelUploadButton.addEventListener('click', hideUploadPreview);
if(confirmUploadButton) confirmUploadButton.addEventListener('click', handleConfirmUpload);
...
function connectWebSocket(requestFeedAfterConnect = false) { 
    if (websocket && (websocket.readyState === WebSocket.OPEN || websocket.readyState === WebSocket.CONNECTING)) { 
        if (requestFeedAfterConnect) {
            // Jika sudah terhubung, langsung minta feed
            websocket.send(JSON.stringify({ type: 'request_feed' }));
        }
        return; 
    } 
    
    // URL dinamis, bisa bekerja di localhost atau server manapun
    const WEBSOCKET_URL = `wss://${window.location.host}`; 

    if (currentView === 'groupchat' && groupChatConnectionStatus) {
    ...
    websocket.onopen = (event) => { 
        ...
        if (!userNickname) { 
            ...
            if (newNick && newNick.trim() !== "") { userNickname = newNick.trim(); } else { userNickname = promptedNick; } 
        ...
        const myCurrentProfilePic = userProfilePictures[userNickname] || (loadUserProfile() || {}).profilePicUrl || DEFAULT_GROUP_AVATAR_URL; 
        userProfilePictures[userNickname] = myCurrentProfilePic;
        
        // Mengirim info profil awal ke server
        ...
        if(appAvatar){ 
            if (myCurrentProfilePic && myCurrentProfilePic !== DEFAULT_GROUP_AVATAR_URL) { 
                appAvatar.style.backgroundImage = `url('${myCurrentProfilePic}')`; appAvatar.textContent = ''; 
            } else { 
                appAvatar.textContent = "💬"; appAvatar.style.backgroundImage = ''; 
            } 
        }
        
        // Jika koneksi dibuat untuk melihat feed, langsung minta datanya
        if (requestFeedAfterConnect) {
            websocket.send(JSON.stringify({ type: 'request_feed' }));
        }
    }; 
    
    // Taruh fungsi onmessage, onclose, onerror Anda yang sudah benar di sini
    // (Kode Anda untuk ini sudah benar, jadi tidak perlu diubah)
    websocket.onmessage = async (event) => { 
        // ... (Kode onmessage Anda yang sudah diperbaiki) ...
