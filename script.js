document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LOGIKA UNTUK ANIMASI BACKGROUND INTERAKTIF ---
    const canvas = document.getElementById('interactive-background');
    if (canvas) {
        const ctx = canvas.getContext('2d');
        let width, height;
        let particles = [];
        const numParticles = 50; // Jumlah titik pada 'ular' animasi
        ...
        function animate() {
            ...
            for (let i = 1; i < particles.length; i++) {
                ...
                const lightness = 50 + (i / numParticles) * 30;
                const lineWidth = (1 - i / numParticles) * 5 + 1;
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.attributeName === 'style') {
                    const isHidden = homeSection.style.display === 'none';
